...
export async function reviewChanges(showDiff: boolean = false): Promise<void> {
  ...
  try {
    ...
    if (!showDiff) {
      console.log(
        `${chalk.cyan('gromit review --show-diff')} ${chalk.gray('# ver preview das mudanças')}`
      );
    }
    console.log(
      `${chalk.cyan('git log --oneline -' + pendingCommits.total)} ${chalk.gray('# ver commits pendentes')}`
    );
  } catch (error) {
